...
renderPlot({
  squads %>%
    ggplot(aes(x = age, y = gls, color = league, shape = league)) +
    geom_point(size = 3) +
    labs(title = "Team Average Age vs. Goals by League",
         x = "Average Age", y = "Goals") +
...
renderPlot({
  squads %>%
    ...
    coord_flip() +
    labs(title = "Top 5 Teams by Goals per Game",
         x = "Squad", y = "Goals per Game") +
...
renderPlot({
  squads %>%
    ggplot(aes(x = poss, y = goals_per_game, color = league)) +
    geom_point() +
    labs(title = "Possession vs Goals per Game", x = "Possession (%)", y = "Goals/Game") +
    theme_minimal()
})
```
